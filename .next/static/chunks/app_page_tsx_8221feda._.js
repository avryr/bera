...
function MetricChart({ dateFrom, dateTo, metric, station, title, unit, minValue = undefined, maxValue = undefined }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "MetricChart.useEffect": ()=>{
            // Fetch bit error rate data
            const berPromise = fetch(`/api/get-chart?metric=bitErrorRate&dateFrom=${dateFrom}&dateTo=${dateTo}`).then({
                "MetricChart.useEffect.berPromise": (response)=>{
                    if (!response.ok) throw new Error('Failed to fetch BER data');
                    return response.json();
                }
            }["MetricChart.useEffect.berPromise"]);
            // Fetch metric data
            const metricPromise = fetch(`/api/get-chart?metric=${metric}&dateFrom=${dateFrom}&dateTo=${dateTo}&station=${station}`).then({
                "MetricChart.useEffect.metricPromise": (response)=>{
                    if (!response.ok) throw new Error(`Failed to fetch ${metric} data`);
                    return response.json();
                }
            }["MetricChart.useEffect.metricPromise"]);
            // Wait for both requests to complete
            Promise.all([
                berPromise,
                metricPromise
            ]).then({
                "MetricChart.useEffect": ([berData, metricData])=>{
                    // Special handling for pressure (convert Pa to hPa)
                    const metricValues = metric === 'barometricPressure' ? metricData.y.map({
                        "MetricChart.useEffect": (value)=>value / 100
                    }["MetricChart.useEffect"]) : metricData.y;
                    setChartData({
                        labels: berData.x,
                        datasets: [
                            {
            ...
            }["MetricChart.useEffect"]).catch({
                "MetricChart.useEffect": (err)=>{
...
_s1(MetricChart, "xzwFLXk0iEu8id7HdsKViSRNrYk=");
_c = MetricChart;
// Specific chart components that use the generic component
function TemperatureChart({ dateFrom, dateTo, station }) {
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(MetricChart, {
        dateFrom: dateFrom,
        dateTo: dateTo,
        metric: "temperature",
        station: station,
...
function Home() {
    ...
    const [station, setStation] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])('CWRU');
...
_s2(Home, "7iRGxrdPJBMEZIQY+4lWO4LEYno=", false, function() {
    return [
        useInitialization
    ];
});
...
__turbopack_context__.k.register(_c1, "TemperatureChart");
__turbopack_context__.k.register(_c2, "HumidityChart");
__turbopack_context__.k.register(_c3, "DewpointChart");
__turbopack_context__.k.register(_c4, "PrecipitationChart");
__turbopack_context__.k.register(_c5, "PressureChart");
